import { BadTreeError } from '../../../errors/engine/data/bad-tree';
...
export class AtlasTree {
	 ...
	 * @returns The tree.
	 */
	private buildTree(position: Vec2 = Vec2.fromValue(0), depth = 0): AtlasTreeNode | undefined {
	...
	constructor(
		...
		if (children.length !== 4 && children.length !== 0) {
			throw new BadTreeError(
				`Trying to create atlas tree node with ${children.length} children (should be 0 or 4)`,
			);
		}
