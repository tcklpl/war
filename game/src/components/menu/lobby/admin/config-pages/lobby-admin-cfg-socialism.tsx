...
import LobbyAdminConfigGroupHp from './components/lobby-admin-cfg-group-hp';
import LobbyAdminConfigOption from './components/lobby-admin-cfg-option';
import LobbyAdminConfigTitle from './components/lobby-admin-cfg-title';
...
const LobbyAdminConfigScreenSocialism = ({ disabled }: { disabled: boolean }) => {
	...
	const [cfg, setCfg] = useState(cloneLobbyState()?.game_config.party_config.socialism);
	useEffect(() => {
		if (!cfg) return;
		modifyLobbyState(s => {
			...
			<Table width='100%'>
				<TableBody>
					...
					<LobbyAdminConfigTitle
						title={`${t('lobby:special_units')} - ${t('lobby:cfg_socialism_su_lula')}`}
					/>
					<LobbyAdminConfigGroupHp
						hp={cfg.special_units.lula.hp}
						maxHp={cfg.special_units.lula.max_hp}
						setHp={hp => {
					...
					<LobbyAdminConfigTitle
						title={`${t('lobby:special_units')} - ${t('lobby:cfg_socialism_su_lenin')}`}
					/>
					<LobbyAdminConfigGroupHp
						hp={cfg.special_units.vladimir_lenin.hp}
						maxHp={cfg.special_units.vladimir_lenin.max_hp}
						setHp={hp => {
					...
					<LobbyAdminConfigTitle
						title={`${t('lobby:special_units')} - ${t('lobby:cfg_socialism_su_stalin')}`}
					/>
					<LobbyAdminConfigGroupHp
						hp={cfg.special_units.joseph_stalin.hp}
						maxHp={cfg.special_units.joseph_stalin.max_hp}
						setHp={hp => {
					...
					<LobbyAdminConfigTitle
						title={`${t('lobby:special_units')} - ${t('lobby:cfg_socialism_su_morus')}`}
					/>
					<LobbyAdminConfigGroupHp
						hp={cfg.special_units.thomas_morus.hp}
						maxHp={cfg.special_units.thomas_morus.max_hp}
						setHp={hp => {
					...
					<LobbyAdminConfigTitle
						title={`${t('lobby:special_units')} - ${t('lobby:cfg_socialism_su_wilson')}`}
					/>
					<LobbyAdminConfigGroupHp
						hp={cfg.special_units.wilson.hp}
						maxHp={cfg.special_units.wilson.max_hp}
						setHp={hp => {
					...
					<LobbyAdminConfigTitle
						title={`${t('lobby:special_units')} - ${t('lobby:cfg_socialism_su_red_god')}`}
					/>
					<LobbyAdminConfigGroupHp
						hp={cfg.special_units.red_god.hp}
						maxHp={cfg.special_units.red_god.max_hp}
						setHp={hp => {
					...
					<LobbyAdminConfigTitle
						title={`${t('lobby:special_abilities')} - ${t('lobby:cfg_socialism_sa_tsar')}`}
					/>
					<LobbyAdminConfigOption
						options={{
							type: 'number',
					...
					<LobbyAdminConfigTitle
						title={`${t('lobby:special_abilities')} - ${t('lobby:cfg_socialism_sa_utopia_island')}`}
					/>
					<LobbyAdminConfigOption
						options={{
							type: 'number',
