...
const LobbyAdminConfigScreenFeudalism = ({ disabled }: { disabled: boolean }) => {
	...
	const [cfg, setCfg] = useState(cloneLobbyState()?.game_config.party_config.feudalism);
	useEffect(() => {
		if (!cfg) return;
		modifyLobbyState(s => {
			...
			<Table width='100%'>
				<TableBody>
					...
					<LobbyAdminConfigTitle
						title={`${t('lobby:special_units')} - ${t('lobby:cfg_feudalism_su_dragon')}`}
					/>
					<LobbyAdminConfigGroupHp
						hp={cfg.special_units.dragon.hp}
						maxHp={cfg.special_units.dragon.max_hp}
						setHp={hp => {
					...
					<LobbyAdminConfigTitle
						title={`${t('lobby:special_abilities')} - ${t('lobby:cfg_feudalism_sa_wall')}`}
					/>
					<LobbyAdminConfigOption
						options={{
							type: 'number',
