            ...
            <Grid container className="server-select-screen" style={{ backgroundColor: palette.background.default }} justifyContent="center" alignContent="center">
                ...
                <Stack spacing={5} width="100%">
                    ...
                    <ServerSelectConnectionInfo
                        open={serverConInfoOpen} setOpen={setServerConInfoOpen}
                        server={selectedServer} serverConMessage={serverConMessage} closeable={serverConCloseable} title={serverConTitle}
                    />

                    <ServerSelectAddServerScreen 
                        open={addServerOpen} setOpen={setAddServerOpen} 
                        servers={servers} setServers={setServers} 
                        serverBeingEdited={serverBeingEdited} setServerBeingEdited={setServerBeingEdited} 
                    />

                    <ServerSelectPasswordPrompt
                        open={passPromptOpen} setOpen={setPassPromptOpen}
                    ...
                    <TableContainer component={Paper} className="server-table">
                        <Table>
                            <TableBody>
                                {servers.sort((a, b) => a.listInfo.listPosition - b.listInfo.listPosition).map(sv => (
                                    <TableRow 
                                        key={sv.listInfo.id} 
                                        onClick={() => setSelectedServer(sv)} selected={selectedServer === sv}
                                        onDoubleClick={() => startConnectionAttempt()}
                                    >
                                        <TableCell>
                                            <Stack spacing={3}>
                                                <Typography>{ sv.status === 'ready' ? `${sv.serverInfo?.name} (${sv.listInfo.localName})` : sv.listInfo.localName }</Typography>
                                                <Typography color={ sv.status === 'error' ? palette.error.main : palette.primary.main }>
                                                    { sv.status === 'ready' ? sv.serverInfo?.description : (
                                                        sv.status === 'error' ? 
                                                            <>
                                                            ...
                                                            : 
                                                            <>
                                                                <CircularProgress size="1em" style={{verticalAlign: "middle", marginRight: "0.5em"}}/>
                                        ...
                                        <TableCell align="center">
                                            {
                                                sv.status === 'ready' ? (
                                                    <>
                                                        <PeopleIcon style={{verticalAlign: "middle", marginRight: "0.5em"}}/>
                    ...
                    <Stack direction="row" className="server-select-buttons" justifyContent="space-around">
                        ...
                        <Button variant="outlined" onClick={() => startConnectionAttempt()} disabled={!selectedServer} startIcon={<SendIcon/>}>
