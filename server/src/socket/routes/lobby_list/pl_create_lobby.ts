    ...
    register(): void {
        this._data.socket.on("createLobby", (name, joinable) => {
            ...
            try {
                ...
                this._log.info(`${this._data.player.username} created lobby "${name}" (current lobbies: ${this._data.gameServer.lobbyManager.lobbies.length} / ${this._data.gameServer.lobbyManager.maxLobbies})`);
                ...
                if (e instanceof OverLimitError) {
                    this._log.info(`${this._data.player.username} failed to create lobby "${name}": The max numbers of lobbies was reached`);
                    errorReason = "full";
                }
                else if (e instanceof UnavailableNameError) {
                    this._log.info(`${this._data.player.username} failed to create lobby "${name}": This lobby name is already in use`);
                    errorReason = "unavailable name";
                }
                else if (e instanceof PlayerAlreadyOwnsALobbyError) {
                    this._log.info(`${this._data.player.username} failed to create lobby "${name}": ${this._data.player.username} already owns a lobby`);
                    errorReason = "already owner";
                }
                else {
                    this._log.err(`${this._data.player.username} failed to create lobby "${name}": Unknown error`);
                    errorReason = "other";
                }
