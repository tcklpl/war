    ...
    register(): void {
        this._data.socket.on('createLobby', (name, joinable) => {
            try {
                ...
                this._log.info(
                    `${this._data.player.username} created lobby "${name}" (current lobbies: ${this._data.gameServer.lobbyManager.lobbies.length} / ${this._data.gameServer.lobbyManager.maxLobbies})`,
                );
                ...
                if (e instanceof OverLimitError) {
                    ...
                    this._log.error(`${this._data.player.username} failed to create lobby "${name}": Unknown error`);
                    errorReason = 'other';
                }
