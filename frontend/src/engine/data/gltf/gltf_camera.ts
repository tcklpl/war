...
export class GLTFCamera {
    ...
    constructor(name: string, type: GLTFCameraTypes, znear: number, zfar: number, specific: GLTFCameraConstructorOptionalArgs) {
        this._name = name;
        this._type = type;
        this._znear = znear;
        this._zfar = zfar;
        ...
        switch (type) {
            case 'perspective':
                if (!specific.aspectRatio || !specific.yfov)
                    throw new BadGLTFFileError('Perspective camera is missing either aspect ratio or yfov properties (or both)');
                this._aspectRatio = specific.aspectRatio;
                this._yfov = specific.yfov;
                break;
            case 'orthographic':
                if (!specific.xmag || !specific.ymag)
                    throw new BadGLTFFileError('Orthographic camera is missing either xmag or ymag properties (or both)');
